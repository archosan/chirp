...
import { createTestLike, createTestPost, createTestUser, countQueries } from "../../tests/helpers";
import {
	createPost,
	deletePost,
	getPost,
...
describe("PostsService", () => {
	...
	describe("getUserPosts", () => {
		...
		it("loads 10 user posts without per-post metadata queries", async () => {
			const author = await createTestUser();
			const viewer = await createTestUser();
			const { comments } = schema;

			for (let i = 0; i < 10; i++) {
				const postId = await createTestPost(author.id, `Profile post ${i}`);
			...
			const posts = await getUserPosts(author.username, viewer.id);
