...
import { notFound, permissionDenied, validationFailed } from "../error";
...
export async function banUser(userId: string, reason: string, adminId: string) {
	...
	if (user.role === "admin") {
		throw permissionDenied("Cannot ban admin users");
	}
...
export async function updateUserRole(userId: string, role: string, adminId: string) {
	...
	if (!validRoles.includes(role)) {
		throw validationFailed("Invalid role");
	}
...
export async function deleteUser(userId: string, adminId: string) {
	...
	if (user.role === "admin") {
		throw permissionDenied("Cannot delete admin users");
	}
