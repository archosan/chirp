...
import { permissionDenied, validationFailed } from "../../../error";
...
describe("AdminHandler", () => {
  ...
  describe("listUsers", () => {
    ...
    it("throws error for non-admin users", async () => {
      ...
      await expect(
        adminHandler.listUsers({ sessionToken: "user-token" }),
  ...
  describe("banUser", () => {
    ...
    it("returns error when ban fails", async () => {
      ...
      vi.mocked(banUser).mockRejectedValue(
        permissionDenied("Cannot ban admin users"),
      );
  ...
  describe("unbanUser", () => {
    ...
    it("returns error when unban fails", async () => {
      ...
      vi.mocked(unbanUser).mockRejectedValue(
        permissionDenied("User is not banned"),
      );
  ...
  describe("deleteUser", () => {
    ...
    it("returns error when delete fails", async () => {
      ...
      vi.mocked(deleteUser).mockRejectedValue(
        validationFailed("Cannot delete admin"),
      );
      ...
      expect(result.success).toBe(false);
      expect(result.error).toContain("Cannot delete admin");
    });
  });
  ...
  describe("deletePostAdmin", () => {
    it("deletes post as admin successfully", async () => {
      ...
      expect(result.success).toBe(true);
      expect(deletePostAdmin).toHaveBeenCalledWith(
        "post-456",
        "Spam content",
        "admin-123",
      );
  ...
  describe("deleteCommentAdmin", () => {
    it("deletes comment as admin successfully", async () => {
      ...
      expect(result.success).toBe(true);
      expect(deleteCommentAdmin).toHaveBeenCalledWith(
        "comment-456",
        "Harassment",
        "admin-123",
      );
  ...
  describe("getDashboardStats", () => {
    ...
    it("throws error for non-admin users", async () => {
      ...
      await expect(
        adminHandler.getDashboardStats({ sessionToken: "user-token" }),
      ).rejects.toThrow("Admin access required");
    });
  });
