...
describe("LikesHandler", () => {
  ...
  describe("togglePostLike", () => {
    ...
    it("returns error when post not found", async () => {
      ...
      vi.mocked(togglePostLike).mockRejectedValue(notFound("Post not found"));
  ...
  describe("getPostLikeStatus", () => {
    ...
    it("throws unauthenticated for invalid token", async () => {
      ...
      await expect(
        likesHandler.getPostLikeStatus({
          sessionToken: "invalid-token",
          postId: "post-456",
        }),
  ...
  describe("getCommentLikeStatus", () => {
    it("returns liked status for valid session", async () => {
      ...
      expect(result.liked).toBe(true);
      expect(getCommentLikeStatus).toHaveBeenCalledWith(
        "comment-456",
        "user-123",
      );
    });
    ...
    it("throws unauthenticated for invalid token", async () => {
      ...
      await expect(
        likesHandler.getCommentLikeStatus({
          sessionToken: "invalid-token",
          commentId: "comment-456",
        }),
      ).rejects.toThrow("Invalid token");
