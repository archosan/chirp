...
import { conflict, notFound, validationFailed } from "../../../error";
...
describe("AuthHandler", () => {
  ...
  describe("getCurrentUser", () => {
    it("returns user data for valid session", async () => {
      ...
      const result = await authHandler.getCurrentUser({
    ...
    it("throws error for invalid session token", async () => {
      ...
      await expect(
        authHandler.getCurrentUser({ sessionToken: "invalid-token" }),
    ...
    it("throws error when user not found", async () => {
      ...
      vi.mocked(getCurrentUser).mockRejectedValue(notFound("User not found"));

      await expect(
        authHandler.getCurrentUser({ sessionToken: "valid-token" }),
  ...
  describe("validateSession", () => {
    it("returns valid response for valid token", async () => {
      ...
      const result = await authHandler.validateSession({
        sessionToken: "valid-token",
      });
    ...
    it("returns invalid response for expired token", async () => {
      vi.mocked(validateSessionToken).mockImplementation(() => {
        throw validationFailed("Token expired");
      });
