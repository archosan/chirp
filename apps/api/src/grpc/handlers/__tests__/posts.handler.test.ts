  ...
  describe("createPost", () => {
    ...
    it("returns error when content exceeds limit", async () => {
      ...
      vi.mocked(createPost).mockRejectedValue(
        validationFailed("Post content exceeds 280 characters"),
      );
      ...
      expect(result.success).toBe(false);
      expect(result.error).toContain("Post content exceeds 280 characters");
    });

    it("returns validation error with trace id when createPost fails validation", async () => {
      vi.mocked(validateSessionToken).mockReturnValue({
        userId: "user-123",
        username: "testuser",
        role: "user",
      ...
      vi.mocked(createPost).mockRejectedValue(
        validationFailed("Post content must be 280 characters or less"),
      );
      ...
      expect(result.error).toContain(
        "Post content must be 280 characters or less",
      );
      expect(result.error).toContain("traceId:");
    });

    it("hides unknown errors behind internal server error", async () => {
      vi.mocked(validateSessionToken).mockReturnValue({
        userId: "user-123",
        username: "testuser",
        role: "user",
      ...
      vi.mocked(createPost).mockRejectedValue(new Error("database exploded"));
  ...
  describe("updatePost", () => {
    ...
    it("returns error when edit window has passed", async () => {
      ...
      vi.mocked(updatePost).mockRejectedValue(
        validationFailed("Edit window has passed"),
      );
      ...
      expect(result.success).toBe(false);
      expect(result.error).toContain("Edit window has passed");
    });
    ...
    it("returns error when user is not the author", async () => {
      ...
      vi.mocked(updatePost).mockRejectedValue(
        validationFailed("You can only edit your own posts"),
      );
      ...
      expect(result.success).toBe(false);
      expect(result.error).toContain("You can only edit your own posts");
    });
  });
  ...
  describe("deletePost", () => {
    ...
    it("returns error when user is not the author", async () => {
      ...
      vi.mocked(deletePost).mockRejectedValue(
        validationFailed("You can only delete your own posts"),
      );
      ...
      expect(result.success).toBe(false);
      expect(result.error).toContain("You can only delete your own posts");
    });
  });
  ...
  describe("getPosts", () => {
    ...
    it("returns posts with isLiked for authenticated user", async () => {
      ...
      vi.mocked(getPosts).mockResolvedValue(
        mockPosts.map((p) => ({ ...p, isLiked: true })),
      );
